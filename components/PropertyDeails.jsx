...
import { FaLocationDot, FaXmark, FaBed, FaBath, FaRulerCombined, FaCheck } from 'react-icons/fa6'

const PropertyDeails = ({property}) => {
  return (
   <main>
            <div
              ...
              <div className="flex flex-col md:flex-row justify-around">
                ...
                >
                  ...
                  <div className="text-2xl font-bold text-blue-500">
                    {property.rates.nightly?(
                        `$${property.rates.nightly.toLocaleString()}`):( <FaTimes className="text-red-700" />)}
                ...
                >
                  ...
                  <div className="text-2xl font-bold text-blue-500">{property.rates.weekly?(
                        `$${property.rates.weekly.toLocaleString()}`):( <FaTimes className="text-red-700" />)}</div>
                </div>
                <div className="flex items-center justify-center mb-4 pb-4 md:pb-0">
                  <div className="text-gray-500 mr-2 font-bold">Monthly</div>
                  <div className="text-2xl font-bold text-blue-500">{property.rates.monthly?(
                        `$${property.rates.monthly.toLocaleString()}`):( <FaTimes className="text-red-700" />)}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mt-6">
              ...
              >
                {property.amenities.map((amenity,index)=>( <li key={index}>
                  <FaCheck className="inline-block text-green-600 mr-2 mt-3" /> {amenity}
                </li>))}
